...
class StructureControl:
    ...
    def merge_control(self):
        ...
        control_merge_all = pd.merge(control_merge_all.reset_index(drop=False), self.l.control_measure_map, left_on="measure_group_id", right_on="measure_group_id")
    ...
    def create_geometry_start_end(self, target_type:str, control_merge_structure:pd.DataFrame):
        ...
        start_nodes = pd.merge(control_merge_structure, self.l.conn_nodes, left_on="object_id", right_on="node_id")[["control_id", "geometry"]]
        end_nodes = pd.merge(control_merge_structure, self.l.conn_nodes, left_on="connection_node_start_id", right_on="node_id")[["control_id", "initial_waterlevel", "geometry"]]
        self.l.control_nodes = pd.merge(start_nodes, end_nodes, left_index=True, right_index=True, suffixes=("_start","_end"))
    ...
    def get_action_values(self, row):
        ...
        if row["target_type"] == "v2_weir":
            ...
            action_values = [
                float(b.split(";")[1].split(" ")[0]) for b in row["action_table"].split("#")
            ]
