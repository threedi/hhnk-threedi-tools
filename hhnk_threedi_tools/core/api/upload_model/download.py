...
def download(
    scenario_name, out_dir, download_max_waterdepth=True, download_raw_results=False
):
    ...
    for scen in scenarios:
        ...
        if download_raw_results:
            ...
            dl.download_file(
                agg_url,
                path=os.path.join(raw_results_dirname_full, "aggregate_results_3di.nc"),
            )
            dl.download_raw_results(
                scen["uuid"],
                pathname=os.path.join(raw_results_dirname_full, "results_3di.nc"),
            )
            dl.download_grid_administration(
                scen["uuid"],
                pathname=os.path.join(raw_results_dirname_full, "gridadmin.h5"),
            )
            log_url = dl.get_logging_link((scen["uuid"]))
            dl.download_file(
                log_url, path=os.path.join(raw_results_dirname_full, "log.zip")
            )
    dl.clear_inbox()
...
if __name__ == "__main__":
    for bui in ["T100", "Radar"]:
        for schematisation in SCHEMATISATIONS:
            model, scenario_name = get_model_and_simulation_name(
                schematisation_name=schematisation, bui=bui
            )
            out_dir = os.path.join(
                BASE_DIR, f"revision {model.revision_number}", "results", scenario_name
            )
            try:
                os.makedirs(out_dir)
            except OSError:
