...
class Simulation:
    ...
    def add_boundaries(self):
        ...
        output_path = Path(self.output_folder.path).joinpath( "tempfiles", filename)
    ...
    def download_sqlite(self, output_folder_sqlite=None):
        ...
        output_path = Path(output_folder_sqlite.path).joinpath("tempfiles", f"model_{self.model_id}.zip")
        output_path.parent.parent.mkdir(exist_ok=True)
        output_path.parent.mkdir(exist_ok=True) #Create parent folder
    ...
    def start(self, extra_name=""):
        ...
        if self.structure_control_valid:
            ...
            apicall_txt = Path(self.output_folder.path) / f"apicall{extra_name}_simid{self.simulation.id}_date{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
