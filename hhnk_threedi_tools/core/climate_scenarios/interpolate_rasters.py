...
def interpoleer_deel(int_frequentie, waterdieptes, frequenties):
    ...
    log_ovfreq = np.log(1./ovfreq)

    #logaritmische interpolatie
    frac = (np.log(1./int_frequentie) - log_ovfreq[onderindex, j, k]) / (log_ovfreq[bovenindex, j, k] - log_ovfreq[onderindex, j, k])

    int_waterdiepte = waterdieptes[onderindex, j, k] + frac * (waterdieptes[bovenindex, j, k] - waterdieptes[onderindex, j, k])
...
def interpoleer_raster_window(idx, part, raster_classes, dem_raster, int_frequentie, frequenties, extra_nodata_value, output_nodata):
    ...
    int_raster_array = interpoleer_deel(int_frequentie, stacked_raster_array, frequenties)
...
def main_interpolate_rasters(T, output_file, rasters, frequenties, output_nodata, dem_path, extra_nodata_value=None):
    ...
    if not output_file.exists:
        ...
        for idx, part in parts.iterrows():
            part, int_raster_array = interpoleer_raster_window(idx=idx,
                                                                part=part,
        ...
        hrt.save_raster_array_to_tiff(output_file=output_file.path,
