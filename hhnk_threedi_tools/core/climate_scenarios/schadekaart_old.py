...
def maak_schadekaart(schade_rasters, frequenties, output_file, dv, n, output_nodata, pixel_factor, damage_meta, damage_nodata):
    ...
    """
    def bereken_jaarlijkse_schade(schade_rasters, frequenties, damage_meta, pixel_factor):
        ...
        with tqdm(total=len(schade_rasters), unit='peilgebied') as pbar: 
            for raster_file, freq in zip(schade_rasters, frequenties):
                ...
                damage_array=None
                damage_array, damage_nodata, damage_meta = wsa.gdal_load_raster(raster_file)

                #Filtering ongewenste cijfers
                damage_array[damage_array<0] = 0.0
                damage_array[damage_array==damage_nodata] = 0.0 #volgende keer proberen
...
#                 schade += [damage_array.sum() / 1e6 * pixel_factor]
    ...
    jaarlijkse_schade, schade = bereken_jaarlijkse_schade(schade_rasters, frequenties, damage_meta, pixel_factor)
    ...
    wsa.save_raster_array_to_tiff(output_file=output_file, 
...
def maak_schade_polygon(peilgebiedenbestand, schade_raster_file, pixel_factor, output_schade_file, output_polder_file):
    """Aggregeer de schaderasters van overlast en plasvorming naar peilgebiedniveau."""
    def bereken_schade_per_peilgebied(peilgebiedenbestand, schade_raster_file, pixel_factor):
        """"""
        #importeer schaderasters
        contante_schade={}
        for schade_type in ['overlast', 'plas']:
            contante_schade[schade_type], nodata, meta = wsa.gdal_load_raster(schade_raster_file[schade_type])
        ...
        for i in tnrange(len(peilgebieden)):
            ...
            mask = wsa.polygon_to_raster(polygon_gdf=poly_gdf, valuefield='val', raster_output_path='', nodata=0, meta=meta, epsg=28992, driver='MEM')
#             mask = geometry_to_raster_mask(poly, extent=meta['bounds'], cellsize=meta['pixel_width'], shape=meta['shape'])
            ...
            for schade_type in ['overlast', 'plas']:
                peilgebieden.at[idx, 'cw_schade_{}'.format(schade_type)] = np.nansum(
                    contante_schade[schade_type][mask>0]) * pixel_factor
        return peilgebieden

    #Bereken de schade per peilgebied
    peilgebieden = bereken_schade_per_peilgebied(peilgebiedenbestand, schade_raster_file, pixel_factor)   

    #Tabel opschonen
    schades = peilgebieden[['id', 'peil_id', 'code', 'name', 'cw_schade_overlast',
                            'cw_schade_plas', 'geometry']].sort_values(by='cw_schade_overlast', ascending=False)
    ...
    schade_per_polder = schades[['name', 'cw_tot', 'cw_ws', 'cw_mv']].groupby('name').sum().sort_values(
...
#                                  meta=meta, epsg=28992, driver='MEM', datatype=gdal.GDT_Byte)
    ...
    schade_peilgebied={}
    for schade_type in ['overlast', 'plas']:     
        schade_peilgebied[schade_type] = np.nansum(contante_schade[schade_type][peilgebied_array==idx]) * pixel_factor
    return (idx, schade_peilgebied)


def maak_schade_polygon_parallel(peilgebiedenbestand, schade_raster_file, pixel_factor, output_schade_file, output_polder_file):
    """Aggregeer de schaderasters van overlast en plasvorming naar peilgebiedniveau."""
    ...
    peilgebied_array = wsa.polygon_to_raster(polygon_gdf=peilgebieden, valuefield='val', raster_output_path='', nodata=255, 
                                         meta=meta, epsg=28992, driver='MEM', datatype=gdal.GDT_Int16)

    # #Bereken de schade per peilgebied
    results = wsa.multiprocess(df=peilgebieden, target_function=bereken_schade_per_peilgebied_parallel, processes=mp.cpu_count()-1,
                       pixel_factor=pixel_factor, meta=meta, boundingbox=boundingbox)
    ...
    for idx, schade_peilgebied in results:
        for schade_type in ['overlast', 'plas']:
            peilgebied_schade.loc[idx,'cw_schade_{}'.format(schade_type)] = schade_peilgebied[schade_type]

    # #Tabel opschonen
    schades = peilgebied_schade[['id', 'peil_id', 'code', 'name', 'cw_schade_overlast',
                            'cw_schade_plas', 'geometry']].sort_values(by='cw_schade_overlast', ascending=False)
