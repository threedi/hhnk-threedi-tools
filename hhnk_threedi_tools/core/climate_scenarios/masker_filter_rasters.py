...
def rasterize_maskerkaart(input_file, mask_plas_path, mask_overlast_path, meta):
    ...
    for mask_type in ['plas', 'overlast']:
        if not os.path.exists(mask_path[mask_type]):
            #Repareer geometry
            temp_geom = maskerkaart_gdf.buffer(0.1).loc[maskerkaart_gdf.case_final == mask_type].unary_union.buffer(-0.1)
            ...
            mask[mask_type] = hrt.gdf_to_raster(gdf=mask_gdf[mask_type], 
                            value_field='val', 
...
def rasterize_maskerkaart_lizard(input_file, mask_all_path, meta, nodata, datatype):
    ...
    if not os.path.exists(mask_all_path):
        ...
        mask_gdf_all = gpd.GeoDataFrame(mask_gdf_all, geometry='geometry', crs=maskerkaart_gdf.crs)
        hrt.gdf_to_raster(gdf=mask_gdf_all, 
                            value_field='val', 
...
def rasterize_geen_schade_lizard(input_file, output_file, meta, nodata, datatype):
    ...
    if not os.path.exists(output_file):
        ...
        filter_gdf['val']=1
        hrt.gdf_to_raster(gdf=filter_gdf, 
                            value_field='val', 
...
def apply_mask_to_raster(mask, raster_file, output_files):
    ...
    def apply_mask_to_raster_array(mask_array, mask_type, raster_array_local, output_file, meta, nodata):
    ...
    for mask_type in ['plas','overlast']:
        if not os.path.exists(output_files[mask_type]):
            apply_mask_to_raster_array(mask_array=mask[mask_type], 
                                         mask_type=mask_type, 
                                         raster_array_local=raster_array.copy(), 
                                         output_file=output_files[mask_type], 
                                         meta=meta, 
                                         nodata=nodata)
        else: print('{} bestond al'.format(output_files[mask_type]))
