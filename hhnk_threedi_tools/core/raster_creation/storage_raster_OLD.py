...
if __name__ == "__main__":
    ...
    storage_lookup_df, _ = storage_lookup.create_storage_lookup(
        rootzone_thickness_cm=rootzone_thickness_cm, storage_unsa_sim_path=None
    )

    folder_schema = FOLDER_TEST

    output_raster = hrt.RasterOld(folder.storage)
    overwrite = True
    nodata = -9999

    groundwlvl_raster = hrt.RasterOld(folder_schema.model.schema_base.rasters.gwlvl_glg)
    dem_raster = hrt.RasterOld(folder_schema.model.schema_base.rasters.dem)
    soil_raster = hrt.RasterOld(folder_schema.model.schema_base.rasters.soil)
    meta_raster = hrt.RasterOld(dem_raster)
