...
@dataclass
class _Data:
    """Container for DAMO conversion data.

    Core output layers (generated by converters, not in raw export):
        - profielgroep, profiellijn, profielpunt, hydroobject_linemerged (from ProfielConverter)
        - waterkering, peilgebiedpraktijk, streefpeil (from PeilgebiedConverter)
        - pomp (from GemaalConverter)
        - kunstwerkopening, regelmiddel (from StuwConverter)
        - doorstroomopening (from BrugConverter)

    Raw export layers (hydroobject, stuw, gemaal, duikersifonhevel, etc.) are loaded dynamically via setattr().
    """

    # Core generated outputs (explicitly typed for IDE support and documentation)
    ...
    waterkering: gpd.GeoDataFrame = field(default_factory=gpd.GeoDataFrame)
    peilgebiedpraktijk: gpd.GeoDataFrame = field(default_factory=gpd.GeoDataFrame)
    streefpeil: gpd.GeoDataFrame = field(default_factory=gpd.GeoDataFrame)
    pomp: gpd.GeoDataFrame = field(default_factory=gpd.GeoDataFrame)
    kunstwerkopening: gpd.GeoDataFrame = field(default_factory=gpd.GeoDataFrame)
    regelmiddel: gpd.GeoDataFrame = field(default_factory=gpd.GeoDataFrame)
