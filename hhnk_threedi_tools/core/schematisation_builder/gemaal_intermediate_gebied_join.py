...
# read layers as a geopandas dataframe
damo_gdf = gpd.read_file(damo_path)
polder_polygon_gdf = gpd.read_file(polder_polygon)
layer_names = fiona.listlayers(damo_path)
...
gdf_peilgebiedpraktijk_linestring.to_file(output_path, driver="GPKG")

# make spatial join between gdf_gemaal and gdf_peilgebiedpraktijk_linestring
...
gdf_gemaal = gdf_gemaal.merge(gemaal_spatial_join[["code", "distance_validation_rule"]], on="code", how="left")

# Loop over gdf_gemaal and print rows where 'distance_validation_rule' is lower than 0.01
for idx, row in gdf_gemaal.iterrows():
    if row["distance_validation_rule"] < 0.01:
        print(row)
