...
def get_and_create_schematisation(
    ...
    schematisations = hrt.call_threedi_api(threedi.api.schematisations_list, name=schematisation_name)

    if schematisations.count == 1:
        uuid_id = hrt.call_threedi_api(threedi.api.organisations_list, unique_id=schematisations.results[0].owner)
        print(f"Schematisation '{schematisation_name}' already exists, skipping creation. owner_uuid: {uuid_id}")
    ...
    schematisation = hrt.call_threedi_api(
        func=threedi.api.schematisations_create,
        data={
...
def upload_sqlite(schematisation, revision, sqlite_path: Union[str, Path]):
    ...
    upload = hrt.call_threedi_api(
        threedi.api.schematisations_revisions_sqlite_upload,
        id=revision.id,
...
def upload_raster(rev_id: int, schema_id: int, raster_type: str, raster_path: Union[str, Path]):
    ...
    raster_create = hrt.call_threedi_api(
        func=threedi.api.schematisations_revisions_rasters_create,
        revision_pk=rev_id,
        schematisation_pk=schema_id,
        data=data,
    )
    ...
    upload = hrt.call_threedi_api(
        func=threedi.api.schematisations_revisions_rasters_upload,
        id=raster_create.id,
        revision_pk=rev_id,
        schematisation_pk=schema_id,
        data=data,
...
def commit_revision(rev_id: int, schema_id: int, commit_message):
    ...
    for wait_time in [0.5, 1.0, 2.0, 10.0, 30.0, 60.0, 120.0, 300.0]:
        revision = hrt.call_threedi_api(
            func=threedi.api.schematisations_revisions_read, id=rev_id, schematisation_pk=schema_id
        )
    ...
    schematisation_revision = hrt.call_threedi_api(
        func=threedi.api.schematisations_revisions_commit,
        id=rev_id,
        schematisation_pk=schema_id,
...
def create_threedimodel(schematisation, revision):
    ...
    for i in range(10):
        rev = hrt.call_threedi_api(
            func=threedi.api.schematisations_revisions_read, id=revision.id, schematisation_pk=schematisation.id
        )
        if rev.is_valid:
            break
        else:
    ...
    threedimodels = hrt.call_threedi_api(
        func=threedi.api.threedimodels_list, revision__schematisation__name=schematisation.name
    )
    ...
    if len(models) > 2:
        ...
        hrt.call_threedi_api(func=threedi.api.threedimodels_delete, id=models[-1]["id"])
