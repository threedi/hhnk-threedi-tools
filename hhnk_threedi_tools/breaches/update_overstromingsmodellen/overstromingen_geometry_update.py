...
for base, df_nodes in nodes_base_missing.groupby("base"):
    ...
    for nid in df_nodes["id"].values:
        ...
        # end node new. This one is the orifice end noe
        ...
        # connection node coming from the channel's connection_node end.
        row["connection_node_start_id"] = int(nid)
        # new connection node.
        row["connection_node_end_id"] = int(new_end_id)

        row["geometry"] = LineString([(p.x, p.y), (end_pt.x, end_pt.y)])
        new_orif_rows.append(row)

        # create the end node from the new_end_id
...
for idx, bc_row in boundary_condition_gdf.iterrows():
    ...
    # create boundary condition connection node id
    boundary_condition_gdf.loc[idx, "connection_node_id"] = end_id

    # copy geometry from connection node id.
