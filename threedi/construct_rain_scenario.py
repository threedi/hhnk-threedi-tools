...
def get_rain_properties(results):
    ...
    # Timestep is list of time passed between timestamp and start (in minutes?) (769) => 12 hours
    timestep = results.nodes.timestamps / 60
    # Calculates rain per node between 0 and timestep at every step of size dt / 60 (so every x hours)
