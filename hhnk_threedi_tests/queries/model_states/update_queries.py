...
def construct_global_settings_control_group_query(global_settings_to_update_df, excluded_ids=[]):
    query = hrt.sql_create_update_case_statement(df=global_settings_to_update_df,
...
def construct_manholes_update_query(manholes_to_update_df, excluded_ids=[]):
    try:
        query = hrt.sql_create_update_case_statement(df=manholes_to_update_df,
                                             layer=manhole_layer,
                                             df_id_col=id_col,
                                             db_id_col=id_col,
...
def construct_weir_height_update_statement(weir_widths_to_update_df, excluded_ids=[]):
    try:
        query = hrt.sql_create_update_case_statement(df=weir_widths_to_update_df,
...
def construct_channels_update_statement(channels_to_update_df, excluded_ids=[]):
    try:
        query = hrt.sql_create_update_case_statement(df=channels_to_update_df,
