...
class TestSchematisation:
    ...
    def hhnk_schematisation_checks(self):
        hhnk_schematisation_checks = HhnkSchematisationChecks(folder=FOLDER_TEST, results={})
        hhnk_schematisation_checks.output_fd.mkdir(parents=True)
        return hhnk_schematisation_checks
    ...
    def test_run_controlled_structures(self, hhnk_schematisation_checks):
        ...
        output_file = hhnk_schematisation_checks.output_fd.gestuurde_kunstwerken
    ...
    def test_run_dewatering_depth(self, hhnk_schematisation_checks):
        hhnk_schematisation_checks.run_dewatering_depth()
        assert hhnk_schematisation_checks.output_fd.drooglegging.exists()

        assert hhnk_schematisation_checks.output_fd.drooglegging.statistics() == {
            "min": -0.76,
            "max": 9.401,
            "mean": 1.332812,
            "std": 1.19355,
    ...
    def test_run_model_checks(self, hhnk_schematisation_checks):
    ...
    def test_run_geometry(self, hhnk_schematisation_checks):
        """TODO empty check"""
        output = hhnk_schematisation_checks.run_geometry_checks()
        assert output.empty

    def test_run_imp_surface_area(self, hhnk_schematisation_checks):
        output = hhnk_schematisation_checks.run_imp_surface_area()
        assert "61 ha" in output

    def test_run_isolated_channels(self, hhnk_schematisation_checks):
        output, some_text = hhnk_schematisation_checks.run_isolated_channels()
        assert output["length_in_meters"].iloc[0] == 168.45

    def test_run_used_profiles(self, hhnk_schematisation_checks):
        output = hhnk_schematisation_checks.run_used_profiles()
    ...
    def test_run_cross_section_duplicates(self, hhnk_schematisation_checks, folder_new: Folders):  # FIXME WVE
        database = folder_new.model.schema_basis_errors.database
        output = hhnk_schematisation_checks.run_cross_section_duplicates(database=database)
        assert output["cross_loc_id"].to_list() == [282, 99999]

    @pytest.mark.skipif(True, reason="Splitter heeft heel veel aanpassingen nodig voordat dit werkt")
    def test_run_cross_section_no_vertex(self, hhnk_schematisation_checks, folder_new: Folders):  # FIXME WVE
        database = folder_new.model.schema_basis_errors.database
        output = hhnk_schematisation_checks.run_cross_section_no_vertex(database=database)
        assert output["cross_loc_id"].to_list() == [320]
        assert output["distance_to_vertex"].to_list() == [1.0]

    def test_create_grid_from_schematisation(self, hhnk_schematisation_checks, folder_new: Folders):
        hhnk_schematisation_checks.create_grid_from_schematisation(
    ...
    def test_run_struct_channel_bed_level(self, hhnk_schematisation_checks):
        # TODO improve test with better result from function
        output = hhnk_schematisation_checks.run_struct_channel_bed_level()
        assert output.iloc[0]["beneden_has_assumption"]

    def test_run_watersurface_area(self, hhnk_schematisation_checks):
        output = hhnk_schematisation_checks.run_watersurface_area()
        assert output[0]["area_diff"][0] == -20

    def test_run_weir_flood_level(self, hhnk_schematisation_checks):
        output = hhnk_schematisation_checks.run_weir_floor_level()
