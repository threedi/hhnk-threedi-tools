...
def test_create_threedimodel(schematisation_pk, revision_id):
    revision = hrt.call_threedi_api(
        func=upload.threedi.api.schematisations_revisions_read, id=revision_id, schematisation_pk=schematisation_pk
    )
    schematisation = hrt.call_threedi_api(func=upload.threedi.api.schematisations_read, id=schematisation_pk)
...
threedimodels = hrt.call_threedi_api(
    func=upload.threedi.api.threedimodels_list, revision__schematisation__name="model_test_v2__0d1d_test"
)
