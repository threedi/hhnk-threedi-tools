...
def get_all_update_queries(global_settings_df=None,
    ...
    try:
        ...
        if global_settings_df is not None and not global_settings_df.empty:
            query_list.append(create_global_settings_rows_update_query(excluded_ids=global_settings_excluded,
            ...
            query_list.append(
                construct_global_settings_control_group_query(global_settings_to_update_df=global_settings_df,
                                                              excluded_ids=update_skip_ids))
        if bank_levels_df is not None and not bank_levels_df.empty:
            query_list.append(create_bank_levels_update_query(new_bank_levels_df=bank_levels_df,
                                                              excluded_ids=bank_levels_excluded))
        if new_manholes_df is not None and not new_manholes_df.empty:
            query_list.append(create_new_manholes_query(new_manholes_df=new_manholes_df,
                                                        excluded_ids=new_manholes_excluded))
        if update_manholes_df is not None and not update_manholes_df.empty:
            query_list.append(construct_manholes_update_query(manholes_to_update_df=update_manholes_df,
                                                              excluded_ids=update_manholes_excluded))
        if weir_width_df is not None and not weir_width_df.empty:
            query_list.append(construct_weir_height_update_statement(weir_widths_to_update_df=weir_width_df,
                                                                     excluded_ids=weir_width_excluded))
        if channels_df is not None and not channels_df.empty:
            query_list.append(construct_channels_update_statement(channels_to_update_df=channels_df,
