from ....variables.database_variables import id_col, conn_node_start_id_col, pump_station_layer, channels_layer, \
culvert_layer, pipe_layer, weir_layer, orifice_layer, conn_node_end_id_col, connection_nodes_layer, \
    cross_sec_def_layer, height_col, width_col, shape_col, impervious_surface_layer, impervious_surface_id_col, \
    impervious_surface_map_layer, cross_sec_loc_layer, conn_node_id_col, channel_id_col, def_id_col, \
    cross_def_id_col, manhole_layer, crest_level_col, surface_layer, surface_id_col, surface_map_layer, \
...
msg_weir_start_lvl_not_close_to_init_waterlvl = "'WARNING: start level ' || {} || ' not close to initial waterlevel '" \
                                                " || {} || ', ' || {}"
msg_channel_strt_end_not_same_init_waterlvl = "'WARNING: initial water level at start and end node are not equal ' " \
                                              "|| {} || ', ' || {}"
msg_orifice_strt_end_not_same_init_waterlvl = "'WARNING: initial water level at start and end node are not equal ' " \
                                              "|| {} || ', ' || {}"
msg_culvert_strt_end_not_same_init_waterlvl = "'WARNING: Initial waterlevel at start en end node not equal' || {} || " \
                                              "', ' || {}"
...
def constr_conn_nodes_without_conn_query():
    query = ''
    layers = [pump_station_layer, channels_layer, culvert_layer, pipe_layer, weir_layer, orifice_layer]
    length = len(layers)
...
model_checks = {
    ...
    'conn_node_without_imp_surface':
        construct_sel_from_where_query(where='{}' +
                                             constr_in_clause(
                                                 'IN', sel=True, frm=True, where=constr_in_clause(
                                                     'NOT IN', sel=True, frm=True)))
        .format(construct_query_head(
            connection_nodes_layer,
            msg_conn_node_without_imp_surface),
            connection_nodes_layer,
    ...
    'channel_without_cross_loc':
        construct_sel_from_where_query(where='{} IS NULL', left_join={'{}': '{} = {}'})
        .format(construct_query_head(
            channels_layer,
            msg_channel_without_cross_loc),
            channels_layer,
            cross_sec_loc_layer,
    ...
    ######################################################################################
    'manhole_without_conn_node':
        construct_sel_from_where_query(where='{} IS NULL', left_join={'{}': '{} = {}'})
        .format(construct_query_head(
            manhole_layer,
            msg_manhole_without_conn_node),
            manhole_layer,
            connection_nodes_layer,
    ...
    'one_d_boundary_without_conn_node':
        ...
        .format(construct_query_head(
            ...
            msg_one_d_boundary_cond_without_conn),
            one_d_boundary_cond_layer,
            connection_nodes_layer,
    ...
    'cross_sec_loc_without_ref_lvl':
        construct_sel_from_where_query(where='{} IS NULL')
        .format(construct_query_head(
            cross_sec_loc_layer,
    ...
    ######################################################################################
    'conn_node_without_conn':
        construct_sel_from_where_query(where=f"""{{}} NOT IN ({constr_conn_nodes_without_conn_query()})""")
        .format(construct_query_head(
            connection_nodes_layer,
            msg_conn_node_without_conn),
            connection_nodes_layer,
            f'{connection_nodes_layer}.{id_col}'
        ),
    ######################################################################################
    'start_lvl_not_close_to_init_waterlvl':
        construct_sel_from_where_query(where='abs({} - {}) > 0.05', left_join={'{}': '{} = {}'})
        .format(construct_query_head(
            pump_station_layer,
    ...
    ######################################################################################
    'undefined_shape_or_width':
        construct_sel_from_where_query(where="""{} IS NULL OR {} IS NULL OR {} = ''""")
        .format(construct_query_head(
            cross_sec_def_layer,
            msg_undefined_shape_or_width),
            cross_sec_def_layer,
            shape_col,
            width_col,
            width_col
    ...
    'weir_start_lvl_not_close_to_init_waterlvl':
        construct_sel_from_where_query(left_join={'{} as conn1': '{} = {}', '{} as conn2': '{} = {}'},
                                       where='abs({} - {}) > 0.05 AND abs({} - {}) > 0.05 AND {} != 15')
        .format(construct_query_head_no_quotes(
            weir_layer,
    ...
    ######################################################################################
    'imp_surface_perc':
        construct_sel_from_where_query(where='{} = 100')
        .format(construct_query_head(
            impervious_surface_map_layer,
            msg_imp_surface_perc),
            impervious_surface_map_layer,
            f'{impervious_surface_map_layer}.{percentage_col}'
        ),
    ######################################################################################
    'action_table_char_count':
        construct_sel_from_where_query(where='length({}) > 1000')
        .format(construct_query_head(
            control_table_layer,
...
class ModelCheck:
    ...
    def __init__(self):
        ...
        self.impervious_surface_map_refers_to_inv_conn_node = model_checks['impervious_surface_map_refers_to_inv_conn_node']
